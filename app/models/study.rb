class Study < ActiveRecord::Base
  ...
  accepts_nested_attributes_for :attachments, :allow_destroy => true, :reject_if => lambda { |a| a[:artwork_id].blank? }

  has_many :image_attachments, :dependent => :destroy
  has_many :images, :through => :image_attachments
  
  accepts_nested_attributes_for :image_attachments, :allow_destroy => true, :reject_if => lambda { |a| a[:image_id].blank? }
end